...
;; more complex ones
(s/def :restaurant/categories
  (let [category-keys (set (keys (util/read-config init/categories-config)))]  ;; TODO: avoid reading file again
    (s/with-gen #(and (set? %) (set/subset? % category-keys) (not= % :all))
      (fn [] (gen/not-empty
             (gen/set (gen/elements (vec category-keys)) {:max-elements 3}))))))

(s/def :restaurant/zip
  (s/with-gen #(and (string? %) (re-matches #"[0-9]{5}" %))
    #(gen/fmap (fn [v] (str/join v))
               (gen/vector (gen/choose 0 9) 5))))

(s/def ::phone
  (s/with-gen #(and (string? %) (re-matches #"[0-9]{3} [0-9]{3}-[0-9]{4}" %))
...
(s/def :restaurant/phone
  (s/or :ph ::phone
        :nl ::not-listed))
(s/def :restaurant/uri
  (s/with-gen #(and (string? %) (re-matches #"^http(s)?://[a-zA-Z0-9]+.[a-z]+(/)?(.*?)" %))
...
(s/def :restaurant/dine-opts
  (let [nsfy-dining-opts #{:dine-in :take-out :delivery :catering :food-truck}]
    (s/with-gen #(and (set? %) (set/subset? % nsfy-dining-opts))
      (fn [] (gen/not-empty (gen/set (gen/elements (vec nsfy-dining-opts))))))))

(s/def :restaurant/opts
  (let [nsfy-rest-opts #{:wifi :late-night :closed :need-verification}]
    (s/with-gen #(and (set? %) (set/subset? % nsfy-rest-opts))
      (fn [] (gen/set (gen/elements (vec nsfy-rest-opts)))))))

(s/def :restaurant/price #{:$ :$$ :$$$ :$$$$})

(s/def :restaurant/coord
  (s/with-gen double?
    (fn [] (gen/double* {:infinite? false :NaN? false}))))
(s/def :restaurant/coords
  (s/coll-of :restaurant/coord :kind vector? :count 2))

(s/def ::restaurant
  (s/keys :req-un [:restaurant/name
                   :restaurant/address
                   :restaurant/city
                   :restaurant/zip
                   :restaurant/phone
